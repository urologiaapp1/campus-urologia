...
import { createFromData } from '@/app/api/admin/program/import/route';
...
export async function POST(request, { params }) {
  ...
  const { data: modules } = await supabase
    .from('modules').select('id, title, position').eq('program_id', id).order('position');
  ...
  const modulesWithContent = await Promise.all((modules || []).map(async (mod) => {
    const { data: lessons } = await supabase
      ...
      .eq('module_id', mod.id).order('position');

    const lessonsWithQuizzes = await Promise.all((lessons || []).map(async (lesson) => {
      const { data: quiz } = await supabase
        .from('quizzes').select('id, title, pass_score').eq('lesson_id', lesson.id).maybeSingle();

      let quizData = null;
      if (quiz) {
        const { data: questions } = await supabase
          .from('quiz_questions').select('id, question, options, position').eq('quiz_id', quiz.id).order('position');
        ...
        quizData = {
          ...
          questions: (questions || []).map((q) => ({
            ...
            correct_index: keyMap[q.id] ?? 0, position: q.position,
          })),
        };
      }
  ...
  const programData = { ...program, modules: modulesWithContent };
  const titleOverride = `Copia de ${program.title}`;

  return createFromData(supabase, programData, titleOverride);
}
